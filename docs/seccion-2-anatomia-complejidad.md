...
Originalmente, el estado era una simple `dataclass`. Cualquier parte del código podía modificar `stt_status` o `llm_status` directamente. Esto era un módulo superficial: no aportaba valor más allá de ser un contenedor de datos.
...
**Por qué es profundo:** El usuario del módulo solo llama a `update_stage`. No necesita saber que existe un sistema de callbacks, ni cómo se disparan las notificaciones para el frontend (WebSockets). La interfaz es mínima, el beneficio es máximo.

---

## 📉 Módulos Superficiales (Shallow Modules)

La IA a menudo tiende a crear demasiados archivos pequeños que solo añaden **Costo Cognitivo**. 

### Autocrítica: El caso de la Captura de Audio
Inicialmente, la IA generó dos archivos separados: `vad_manager.py` y `audio_input.py`. Cada uno tenía menos de 30 líneas de código. Para capturar una frase, el `VoicePipeline` tenía que coordinar manualmente ambos módulos, lo que aumentaba la complejidad del orquestador.
...
async def capture_utterance(self) -> bytes:
    # Oculta la lógica de:
    # 1. Buffering de chunks
    # 2. Detección de inicio de habla
    # 3. Contador de chunks de silencio para el final
    ...
```

Al ensanchar la profundidad de `AudioCapture`, eliminamos la necesidad de que el pipeline entienda qué es un "chunk de 30ms" o un "umbral de silencio". 
